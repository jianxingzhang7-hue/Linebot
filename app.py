...
def check_market_status():
    ...
    try:
        ...
        if market.empty: return True, 0, 0
        
        # 修正：確保抓取的是單一數值
        current_index = float(market['Close'].iloc[-1])
        ma5_week = float(market['Close'].rolling(5).mean().iloc[-1])
...
def ashin_strategy(ticker):
    ...
    try:
        ...
        df = yf.download(stock_id, period="4mo", progress=False)
        if df.empty or len(df) < 25: return None

        # 1. 技術面：均線多頭排列 MA5 > MA10 > MA24
        price = float(df['Close'].iloc[-1])
        ma5 = float(df['Close'].rolling(5).mean().iloc[-1])
        ma10 = float(df['Close'].rolling(10).mean().iloc[-1])
        ma24 = float(df['Close'].rolling(24).mean().iloc[-1])
        ...
        avg_amount_5d = float(df['Amount'].tail(5).mean())
